...
//		str.append("token:");//前缀
//		UserAgent ag=UserAgentUtil.parseUserAgentString(agent);
//		if(agent.getOperatingSystem().isMobileDevice()){sat.append("MOBILE-")}
//		else{str.append("PC-")}
...
//			sb.append(user.getUserCode() + "-");
   ...
   //保存token信息
	...
	@Override
	public void delete(String token) {//删除有效token到redis删除保存的token
	...
	public String replaceToken(String agent, String token)
		...
		long ttl = redisAPI.ttl(token);// token有效期（剩余秒数 ），拿到的json字符串
	...
	public boolean validate(String agent, String token) {
		...
		try {
			...
			if(passed>this.SESSION_TIMEOUT*1000)
					return false;
			//传入的agent进行比对，判断是不是等于agentMD5
			agentMD5 = tokenDetails[4];//传入过来的token
			if(MD5.getMd5(agent, 6).equals(agentMD5))//在进行加密一次
				return true;
		} catch (ParseException e) {
			return false;
		}
