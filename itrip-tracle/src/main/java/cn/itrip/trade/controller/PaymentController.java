...
public class PaymentController {
	...
	public String prePay(@PathVariable String orderNo, ModelMap model) {//订单号orderNo确认信息在页面显示model
		try {
			ItripHotelOrder order = orderService.loadItripHotelOrder(orderNo);//查找酒店订单
			if (!EmptyUtils.isEmpty(order)) {
                System.out.println("进去支付宝");
				model.addAttribute("hotelName", order.getHotelName());//酒店名称
				model.addAttribute("roomId", order.getRoomId());//房间编号
				model.addAttribute("count", order.getCount());//总数
				model.addAttribute("payAmount", order.getPayAmount());//金额
				return "pay";
			}else
				return "notfound";
		} catch (Exception e) {
	...
	public void pay(
			...
			@ApiParam(name="WIDtotal_amount",value="订单金额",required=true)//三个api请求参数
		...
		// 封装请求支付信息
		AlipayTradeWapPayModel model = new AlipayTradeWapPayModel();//使用mode吧请求参数进行封装
		model.setOutTradeNo(WIDout_trade_no);//订单编号
		model.setSubject(WIDsubject);//订单名称
		model.setTotalAmount(WIDtotal_amount);//订单金额
		model.setTimeoutExpress(timeout_express);
		model.setProductCode(product_code);
		alipay_request.setBizModel(model);
		// 设置异步通知地址
		alipay_request.setNotifyUrl(alipayConfig.getNotifyUrl());//告诉请求通知
		// 设置同步地址
	...
	/**
	 * 导步通知，跟踪支付状态变更,此代码可以直接复制，对通知进行处理
	 */
	@ApiIgnore
	...
	public void callback(HttpServletRequest request,
		...
		try {
			...
			System.out.println("支付宝return1"+verify_result);
