...
public class WxPaymentController {
    ...
    public Dto createQcCode(@PathVariable String orderNo, HttpServletResponse response) {
        ...
        HashMap<String, Object> result = new HashMap<String, Object>();//这里必须为字符串才能转换成xml
        ...
        try {
            ...
            //这里如果return_code为success通讯成功，全部为success则完全成功
...
//我们给微信支付平台的是xml数据，平台给我们的也是xml数据，我们所做的就是将这个xml转换解析成字符串
    ...
    public Map<String, String> paymentCallBack(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            ...
            String s;
            BufferedReader in = new BufferedReader(new InputStreamReader(inputStream, "UTF-8"));//拿到xml数据
            ...
            if (flag) {//签名判断，判断数据是否合法
                String returnCode = params.get("return_code");
                logger.info("3.returnCode:" + returnCode);
                if (returnCode.equals("SUCCESS")) {
                    String transactionId = params.get("transaction_id");
                    String outTradeNo = params.get("out_trade_no");//获取订单号
                    if (!orderService.processed(outTradeNo)) {//判断如果该订单号不是支付成功则改为支付成功
                        System.out.println("微信交易成功");
