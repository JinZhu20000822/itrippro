...
public class OrderServiceImpl implements OrderService {
	...
	public ItripHotelOrder loadItripHotelOrder(String orderNo) throws Exception {
		...
		logger.debug("加载订单："+orderNo);//查看是否有订单
		Map<String, Object> param=new HashMap();//用map进行入参
		param.put("orderNo", orderNo);
		List<ItripHotelOrder> orders=itripHotelOrderMapper.getItripHotelOrderListByMap(param);//查找酒店订单
		if(orders.size()==1){
			return orders.get(0);//返回0是查找到
		}else
			return null;
	}
	...
	public void paySuccess(String orderNo, int payType,String tradeNo) throws Exception {
		...
		itripHotelOrderMapper.updateItripHotelOrder(itripHotelOrder);//更新库存
