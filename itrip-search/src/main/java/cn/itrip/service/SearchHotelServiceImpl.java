...
public class SearchHotelServiceImpl implements SearchHotelService {
    ...
    public Page<ItripHotelVO> searchItripHotelPage(SearchHotelVO vo, Integer pageNo, Integer pageSize) throws Exception {
        SolrQuery query = new SolrQuery("*:*");//创建查询数据对象（便于设置查询条件）
        StringBuffer tempQuery = new StringBuffer();
        int tempFlag = 0;
        if (EmptyUtils.isNotEmpty(vo)) { //按照目的地搜索
            if (EmptyUtils.isNotEmpty(vo.getDestination())) {
                tempQuery.append(" destination :" + vo.getDestination());
                tempFlag = 1;
            }
            if (EmptyUtils.isNotEmpty(vo.getHotelLevel())) {//按照酒店星级搜索
                query.addFilterQuery("hotelLevel:" + vo.getHotelLevel() + "");
            }
            if (EmptyUtils.isNotEmpty(vo.getKeywords())) { //按照关键词进行搜索
                if (tempFlag == 1) {
            ...
            if (EmptyUtils.isNotEmpty(vo.getFeatureIds())) {//按照酒店特色搜索
            ...
            if (EmptyUtils.isNotEmpty(vo.getTradeAreaIds())) { //按照酒店商圈搜索
                StringBuffer buffer = new StringBuffer("(");
                int flag = 0;
            ...
            if (EmptyUtils.isNotEmpty(vo.getMaxPrice())) {   //按照酒店价格区间搜索
            ...
            if (EmptyUtils.isNotEmpty(vo.getAscSort())) { //升序 排序
                query.addSort(vo.getAscSort(), SolrQuery.ORDER.asc);
            }

            if (EmptyUtils.isNotEmpty(vo.getDescSort())) { //降序 排序
